...
<template>
  <div class="authorization-vue frame-page h-panel">
    <div class="h-panel-bar">
      ...
      <span class="dark2-color">
        保存示例是存储在Local Storage中，可通过清空 Local Storage 属性 SYS_CONFIG_MENU 重置菜单
      </span>
    </div>
    <div class="h-panel-body">
      <div class="authorization-frame">
        <div class="role-container">
          <div class="role-item" :class="{ selected: role === r }" v-for="r of roles" :key="r.id" @click="changeRole(r)">
        ...
        <div class="menu-container">
          <div style="margin-bottom: 10px;">
            <Button @click="$refs.menu.chooseAll() && $refs.menu.expandAll()" color="primary" size="s">
              全选
            </Button>
            <Button @click="menus = []" size="s">清空</Button>
          </div>
          <Tree :option="menuOption" choose-mode="some" multiple ref="menu" v-model="menus" />
        </div>
        <div class="users-container">
          <div style="margin-bottom: 10px;">
            <Button @click="$refs.user.chooseAll() && $refs.user.expandAll()" color="primary" size="s">
              全选
            </Button>
            <Button @click="users = []" size="s">清空</Button>
          </div>
          <Tree :option="userOption" multiple ref="user" v-model="users"></Tree>
        </div>
      </div>
      <div class="text-center">
        <Button @click="save" color="primary">保存</Button>
