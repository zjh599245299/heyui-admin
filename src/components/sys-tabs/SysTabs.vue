  ...
  methods: {
    ...
    trigger(key, data, event) {
      if (key === 'closeAll') {
        ...
        const item = this.tagList[this.nowIndex];
        if (key === 'closeOther') {
    ...
    init() {
      ...
      log.info('this.tagList', this.tagList);
    ...
    gotoTab(item) {
      ...
      const routeObj = { path: item.meta.key, name: meta.title, query, params, meta: meta || {} };
